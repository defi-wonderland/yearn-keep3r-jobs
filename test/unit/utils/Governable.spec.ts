...
describe('Governable', () => {
  ...
  describe('constructor', () => {
    it('should revert when given zero address', async () => {
      await expect(governableFactory.deploy(constants.ZERO_ADDRESS)).to.be.revertedWith('ZeroAddress()');
    });
  });
  ...
  context('after deployed', () => {
    ...
    describe('setPendingGovernor', () => {
      ...
      it('should revert when given zero address', async () => {
        await expect(governable.connect(governor).setPendingGovernor(constants.ZERO_ADDRESS)).to.be.revertedWith('ZeroAddress()');
      });
