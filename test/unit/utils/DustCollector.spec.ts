...
import { DustCollectorForTest, DustCollectorForTest__factory, IERC20 } from '@typechained';
import { wallet, constants } from '@utils';
...
describe('DustCollectorForTest', () => {
  ...
  beforeEach(async () => {
    ...
    await wallet.setBalance({ account: dust.address, balance: tenTokens });
  });
  ...
  describe('sendDust', () => {
    ...
    it('should revert if the receiver is the zero address', async () => {
      await expect(dust.sendDust(constants.ETH_ADDRESS, oneEth, constants.ZERO_ADDRESS)).to.be.revertedWith('ZeroAddress()');
    });
    ...
    context('when the function is called with the correct parameters', () => {
      ...
      it('should transfer ETH successfully', async () => {
        await dust.sendDust(constants.ETH_ADDRESS, oneEth, randomAddress);
        expect(await ethers.provider.getBalance(randomAddress)).to.equal(oneEth);
      });

      it('should emit an event if the transfer is successful', async () => {
        await expect(dust.sendDust(constants.ETH_ADDRESS, oneEth, randomAddress))
          .to.emit(dust, 'DustSent')
          .withArgs(constants.ETH_ADDRESS, oneEth, randomAddress);
      });
